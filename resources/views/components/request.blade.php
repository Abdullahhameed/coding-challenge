    ...
    @if ($mode == 'sent')
        @foreach ($sendRequests as $sendRequest)
            <div class="d-flex justify-content-between mt-1">
                ...
                <div>
                    <button onclick="removeConnection('{{ $sendRequest->id }}')" id="cancel_request_btn_sent"
                        class="btn btn-danger me-1" onclick="">Withdraw Request</button>
                </div>
            </div>
        @endforeach
    @else
        @foreach ($receivedRequests as $receivedRequest)
            <div class="d-flex justify-content-between mt-1">
                ...
                <div>
                    <button onclick="acceptRequest('{{ $receivedRequest->id }}')" id="accept_request_btn_"
                        class="btn btn-primary me-1">Accept</button>
                </div>
            </div>
        @endforeach
    @endif
